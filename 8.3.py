"""
Создайте в программе "Моя зверюшка" своего рода "черный ход" - способ
увидеть точные значения числовых атрибутов объекта. Сделайте в меню
секретный пункт, который подсказка не будет отражать, и, если пользователь
его выберет, выводит объект на экран (для этого в классе Critter должен
быть реализован специальный метод __str__()).
"""
